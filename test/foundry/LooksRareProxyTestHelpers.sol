...
abstract contract LooksRareProxyTestHelpers {
    ...
    function validBAYCId9483Order() internal pure returns (BasicOrder[] memory orders) {
        orders = new BasicOrder[](1);

        uint256[] memory amounts = new uint256[](1);
        amounts[0] = 1;

        orders[0].signer = 0x17331428346E388f32013e6bEc0Aba29303857FD;
        orders[0].collection = BAYC;
        orders[0].collectionType = CollectionType.ERC721;
        ...
        orders[0].amounts = amounts;
        orders[0].price = 73.88 ether;
        orders[0].currency = WETH;
        orders[0].startTime = 1662683712;
        orders[0].endTime = 1663288508;
        orders[0]
